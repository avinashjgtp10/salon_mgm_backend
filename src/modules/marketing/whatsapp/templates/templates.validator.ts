...
export const validateCreateTemplate = (req: Request, _res: Response, next: NextFunction) => {
  try {
    ...
    // FIX: buttons may arrive as a JSON string when sent via FormData — parse it first
    if (typeof b.buttons === 'string') {
      try {
        b.buttons = JSON.parse(b.buttons)
      } catch {
        b.buttons = []
      }
    }
    // Default buttons to empty array if not provided
    if (b.buttons === undefined || b.buttons === null) {
      b.buttons = []
    }
    ...
    // Filter out incomplete buttons rather than hard-rejecting the whole request
