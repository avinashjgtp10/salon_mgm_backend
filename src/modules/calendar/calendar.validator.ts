...
const isOptionalPositiveNum = (v: unknown): boolean =>
    v === undefined || v === null || (typeof v === "number" && Number.isFinite(v) && v >= 0);
const isISODatetime = (v: unknown): boolean =>
    ...
    try {
        ...
        for (const f of ["branch_id", "client_id", "staff_id", "service_id"]) {
            if (!isOptionalUUID(b[f]))
                throw new AppError(400, `${f} must be a UUID`, "VALIDATION_ERROR");
        }
...
export const validateCheckoutAppointment = (
    req: Request,
    _res: Response,
    next: NextFunction
): void => {
    try {
        ...
        for (let i = 0; i < b.items.length; i++) {
            ...
            if (!VALID_ITEM_TYPES.includes(item.item_type))
                throw new AppError(400, `items[${i}].item_type must be one of: ${VALID_ITEM_TYPES.join(", ")}`, "VALIDATION_ERROR");
            ...
            if (!isPositiveInt(item.quantity))
                throw new AppError(400, `items[${i}].quantity must be a positive integer`, "VALIDATION_ERROR");

            if (!isOptionalPositiveNum(item.unit_price))
                throw new AppError(400, `items[${i}].unit_price must be >= 0`, "VALIDATION_ERROR");

            if (!isOptionalPositiveNum(item.discount_amount))
                throw new AppError(400, `items[${i}].discount_amount must be >= 0`, "VALIDATION_ERROR");

            if (!isOptionalUUID(item.staff_id))
                throw new AppError(400, `items[${i}].staff_id must be a UUID`, "VALIDATION_ERROR");
        }

        for (const f of ["discount_amount", "tip_amount", "tax_amount"]) {
            if (!isOptionalPositiveNum(b[f]))
                throw new AppError(400, `${f} must be a non-negative number`, "VALIDATION_ERROR");
        }
