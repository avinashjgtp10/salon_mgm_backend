...
export const membershipsRepository = {
    ...
    async list(_q: MembershipsListQuery): Promise<{ items: Membership[]; total: number }> {
        const { rows } = await pool.query(
            `${SELECT_WITH_SERVICES} GROUP BY m.id ORDER BY m.created_at DESC`
        );
    ...
    async update(id: string, data: UpdateMembershipDTO): Promise<Membership | null> {
        ...
        try {
            ...
            if (data.includedServices) {
                await client.query(
                    `DELETE FROM membership_services WHERE membership_id = $1`, [id]
                );
    ...
    async delete(id: string): Promise<boolean> {
        const { rowCount } = await pool.query(
            `DELETE FROM memberships WHERE id = $1`, [id]
        );
        return (rowCount ?? 0) > 0;
    },
};
