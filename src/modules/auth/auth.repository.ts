...
export const authRepository = {
  ...
  async markUserVerified(userId: string) {
    const { rows } = await safeQuery(() =>
      pool.query(
        `UPDATE users SET is_verified = TRUE, updated_at = NOW() WHERE id = $1 RETURNING id, email, is_verified, updated_at`,
        [userId],
      ),
    );
    return rows[0];
  ...
  async updatePassword(userId: string, passwordHash: string) {
    await safeQuery(() =>
      pool.query(`UPDATE users SET password_hash = $1, updated_at = NOW() WHERE id = $2`, [passwordHash, userId]),
    );
  },
  ...
  async createOtpVerification(data: {
    ...
    const { rows } = await safeQuery(() =>
      pool.query(
        `INSERT INTO otp_verifications (user_id, otp_code, purpose, expires_at) VALUES ($1, $2, $3, $4) RETURNING id, user_id, purpose, expires_at, is_used, created_at`,
  ...
  async findLatestOtp(userId: string, purpose: string) {
    const { rows } = await safeQuery(() =>
      pool.query(
        `SELECT * FROM otp_verifications WHERE user_id = $1 AND purpose = $2 ORDER BY created_at DESC LIMIT 1`,
        [userId, purpose],
      ),
    );
    return rows[0] || null;
  ...
  async markOtpUsed(otpId: string) {
    await safeQuery(() =>
      pool.query(`UPDATE otp_verifications SET is_used = TRUE WHERE id = $1`, [otpId]),
    );
  },
  ...
  async findRefreshToken(token: string) {
    const { rows } = await safeQuery(() =>
      pool.query(`SELECT * FROM refresh_tokens WHERE token = $1 LIMIT 1`, [token]),
    );
    return rows[0] || null;
  },
  ...
  async markOnboardingComplete(userId: string) {
    await safeQuery(() =>
      pool.query(`UPDATE users SET is_onboarding_complete = true WHERE id = $1`, [userId]),
    );
  },
  ...
  async findGoogleIdentity(providerUserId: string) {
    const { rows } = await safeQuery(() =>
      pool.query(
        `SELECT u.* FROM user_identities ui JOIN users u ON u.id = ui.user_id WHERE ui.provider='google' AND ui.provider_user_id=$1 LIMIT 1`,
        [providerUserId],
      ),
    );
    return rows[0] || null;
  ...
  async createUserFromGoogle(profile: { email: string | null; fullName: string | null; avatarUrl: string | null }) {
    ...
    const { rows } = await safeQuery(() =>
      pool.query(
        `INSERT INTO users(email, full_name, first_name, last_name, avatar_url, is_verified, role, password_hash) VALUES($1,$2,$3,$4,$5,TRUE,'client','') RETURNING *`,
  ...
  async attachGoogleIdentity(userId: string, profile: { providerUserId: string; email: string | null }) {
    const { rows } = await safeQuery(() =>
      pool.query(
        `INSERT INTO user_identities(user_id, provider, provider_user_id, email) VALUES($1,'google',$2,$3) ON CONFLICT (provider, provider_user_id) DO NOTHING RETURNING *`,
  ...
  async updateUserBasics(userId: string, profile: { fullName: string | null; avatarUrl: string | null; email: string | null }) {
    const { rows } = await safeQuery(() =>
      pool.query(
        `UPDATE users SET full_name = COALESCE($1, full_name), avatar_url = COALESCE($2, avatar_url), email = COALESCE($3, email), is_verified = TRUE, updated_at = NOW() WHERE id=$4 RETURNING *`,
