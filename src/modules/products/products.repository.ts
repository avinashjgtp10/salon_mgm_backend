...
export const productsRepository = {
    async findById(id: string, salonId: string): Promise<Product | null> {
        const { rows } = await pool.query(
            `SELECT * FROM products WHERE id = $1 AND salon_id = $2`,
            [id, salonId]
        );
        return rows[0] || null;
    },
    ...
    async update(id: string, patch: UpdateProductBody, salonId: string): Promise<Product> {
        ...
        if (keys.length === 0) {
            const { rows } = await pool.query(
                `SELECT * FROM products WHERE id = $1 AND salon_id = $2`, [id, salonId]
            );
            return rows[0];
        }
        ...
        values.push(salonId);
        const { rows } = await pool.query(
            `UPDATE products SET ${setParts.join(", ")} WHERE id = $${values.length - 1} AND salon_id = $${values.length} RETURNING *`,
            values
        );
        return rows[0];
    },
