...
// Create payroll_entries table — one row per staff member per payroll period
// (weekly/bi-weekly/monthly/custom). Net pay and pending amount are derived
// (base+commission+tips+bonus-salary_advance-deductions, and net_pay-paid_amount)
// rather than stored, so they never drift from the source amounts (safe, idempotent).
setImmediate(() => {
  pool.query(`DO $$ BEGIN
  ...
  EXCEPTION WHEN duplicate_object THEN NULL; END $$;`)
    ...
    `))
    .catch((err: any) => console.warn('⚠️  payroll_entries table migration:', err.message));
});

// Salary advances are stored as individual transactions so multiple advances
// within one payroll period can be tracked, edited, and summed safely.
setImmediate(() => {
  pool.query(`
