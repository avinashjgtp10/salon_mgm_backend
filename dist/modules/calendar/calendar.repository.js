...
exports.calendarRepository = {
    ...
    async list(filters) {
        ...
        const { rows: countRows } = await database_1.default.query(`SELECT COUNT(*) FROM appointments ${where}`, values);
        const total = parseInt(countRows[0].count, 10);
        const { rows } = await database_1.default.query(`SELECT * FROM appointments ${where}
    ...
    async hasConflict(params) {
        ...
        const { rows } = await database_1.default.query(`SELECT id FROM appointments
       WHERE staff_id = $1
    ...
    async update(id, patch) {
        ...
        if (hasScheduled || hasDuration) {
            const scheduledExpr = hasScheduled
                ? `$${keys.indexOf("scheduled_at") + 1}::timestamptz`
                : `scheduled_at`;
            const durationExpr = hasDuration
                ? `($${keys.indexOf("duration_minutes") + 1}::text || ' minutes')::interval`
                : `(duration_minutes::text || ' minutes')::interval`;
            setParts.push(`ends_at = ${scheduledExpr} + ${durationExpr}`);
        }
        setParts.push(`updated_at = NOW()`);
        values.push(id);
        const { rows } = await database_1.default.query(`UPDATE appointments
       SET ${setParts.join(", ")}
    ...
    async linkSale(appointmentId, saleId) {
        await database_1.default.query(`UPDATE appointments
       SET sale_id = $1, status = 'completed', updated_at = NOW()
       WHERE id = $2`, [saleId, appointmentId]);
    },
};
