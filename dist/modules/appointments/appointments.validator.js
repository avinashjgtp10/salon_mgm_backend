...
const isISODatetime = (v) => typeof v === "string" && !isNaN(new Date(v).getTime());
...
const validateCreateAppointment = (req, _res, next) => {
    try {
        ...
        if (!isOptionalUUID(b.client_id))
            throw new error_middleware_1.AppError(400, "client_id must be a UUID", "VALIDATION_ERROR");
        if (!isOptionalUUID(b.staff_id))
            throw new error_middleware_1.AppError(400, "staff_id must be a UUID", "VALIDATION_ERROR");
        if (!isOptionalUUID(b.service_id))
            throw new error_middleware_1.AppError(400, "service_id must be a UUID", "VALIDATION_ERROR");
        ...
        if (new Date(b.scheduled_at) < new Date())
            throw new error_middleware_1.AppError(400, "scheduled_at cannot be in the past", "VALIDATION_ERROR");
        ...
        if (b.status !== undefined && !VALID_STATUSES.includes(b.status))
            throw new error_middleware_1.AppError(400, `status must be one of: ${VALID_STATUSES.join(", ")}`, "VALIDATION_ERROR");
    ...
    try {
        ...
        for (const [i, item] of b.items.entries()) {
            if (!VALID_ITEM_TYPES.includes(item.item_type))
                throw new error_middleware_1.AppError(400, `items[${i}].item_type is invalid`, "VALIDATION_ERROR");
            if (typeof item.name !== "string" || item.name.trim().length === 0)
                throw new error_middleware_1.AppError(400, `items[${i}].name is required`, "VALIDATION_ERROR");
            if (typeof item.quantity !== "number" || item.quantity <= 0)
                throw new error_middleware_1.AppError(400, `items[${i}].quantity must be positive`, "VALIDATION_ERROR");
            if (typeof item.unit_price !== "number" || item.unit_price < 0)
                throw new error_middleware_1.AppError(400, `items[${i}].unit_price must be non-negative`, "VALIDATION_ERROR");
        }
        return next();
    }
    catch (err) {
...
//# sourceMappingURL=appointments.validator.js.map