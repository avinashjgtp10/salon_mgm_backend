...
exports.campaignWorker = new bullmq_1.Worker('wa-campaign-messages', async (job) => {
    ...
    const { rows: [campaign] } = await database_1.default.query(`
      SELECT c.*, t.name AS template_name, t.body_text, t.language,
             t.header_type, t.header_media_id
      FROM wa_campaigns c
    ...
    for (const contact of contacts) {
        try {
            ...
            const res = await axios_1.default.post(`${WA_BASE_URL}/${WA_API_VERSION}/${waConfig.phone_number_id}/messages`, {
            ...
            await database_1.default.query(`UPDATE wa_campaign_contacts SET status='SENT', wamid=$1, sent_at=NOW(), updated_at=NOW() WHERE id=$2`, [wamid, contact.id]);
            sent++;
        }
        catch (err) {
            ...
            await database_1.default.query(`UPDATE wa_campaign_contacts SET status=$1, error_code=$2, error_message=$3, updated_at=NOW() WHERE id=$4`, [isBlocked ? 'BLOCKED' : 'FAILED', code, msg, contact.id]);
    ...
    if (parseInt(counts.done) >= parseInt(counts.total)) {
        await database_1.default.query(`UPDATE wa_campaigns SET status='COMPLETED', completed_at=NOW(), updated_at=NOW() WHERE id=$1`, [campaignId]);
    }
}, { connection: whatsapp_queue_1.redisConnection, concurrency: 1 });
