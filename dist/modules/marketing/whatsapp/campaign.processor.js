...
exports.campaignWorker = new bullmq_1.Worker('wa-campaign-messages', async (job) => {
    ...
    if (!camp || camp.status === 'PAUSED')
        return;
    const waConfig = await whatsapp_service_1.whatsappService.getSalonConfig(salonId);
    const { rows: [campaign] } = await database_1.default.query(`
      SELECT c.*, t.name as template_name, t.body_text, t.language, t.header_type, t.header_media_id
    ...
    for (const contact of contacts) {
        try {
            ...
            const components = [];
            if (campaign.header_type && campaign.header_type !== 'none' && campaign.header_type !== 'text' &&
                campaign.header_media_id && !campaign.header_media_id.startsWith('4:')) {
                const mt = campaign.header_type;
                components.push({ type: 'header', parameters: [{ type: mt, [mt]: { id: campaign.header_media_id } }] });
            }
            if (matches.length > 0) {
                const variables = contact.variables || {};
                const params = matches.map((_, i) => ({
                    type: 'text',
                    text: String(variables[`var${i + 1}`] || variables[String(i + 1)] || (i === 0 && contact.name ? contact.name : null) || `Value${i + 1}`),
                }));
            ...
            await database_1.default.query(`UPDATE wa_campaign_contacts SET status='SENT', wamid=$1, sent_at=NOW() WHERE id=$2`, [wamid, contact.id]);
            sent++;
        }
        catch (err) {
            ...
            await database_1.default.query(`UPDATE wa_campaign_contacts SET status=$1, error_code=$2, error_message=$3 WHERE id=$4`, [isBlocked ? 'BLOCKED' : 'FAILED', code, msg, contact.id]);
            isBlocked ? blocked++ : failed++;
        }
        await new Promise(r => setTimeout(r, 200));
    }
    await database_1.default.query(`UPDATE wa_campaigns SET sent_count=sent_count+$1, failed_count=failed_count+$2, blocked_count=blocked_count+$3 WHERE id=$4`, [sent, failed, blocked, campaignId]);
    const { rows: [counts] } = await database_1.default.query(`
      SELECT COUNT(*) FILTER (WHERE status != 'PENDING') as done, COUNT(*) as total
      FROM wa_campaign_contacts WHERE campaign_id = $1
    `, [campaignId]);
    if (parseInt(counts.done) >= parseInt(counts.total)) {
        await database_1.default.query(`UPDATE wa_campaigns SET status='COMPLETED', completed_at=NOW() WHERE id=$1`, [campaignId]);
    }
